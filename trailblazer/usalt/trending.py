# -*- coding: utf-8 -*-
""" Get USALT data for trending."""

from trailblazer.usalt import files


def parse_usalt_analysis(usalt_config_raw: dict, qcmetrics_raw: dict, sampleinfo_raw: dict) -> dict:
    """Parse the output analysis files from USALT for adding info
    to trend database

    Args:
        usalt_config_raw (dict): raw YAML input from USALT analysis config file
        qcmetrics_raw (dict): raw YAML input from USALT analysis qc metric file
        sampleinfo_raw (dict): raw YAML input from USALT analysis qc sample info file
    Returns:
        dict: parsed data
    """
    outdata = _define_output_dict()

    _config(usalt_config_raw, outdata)
...
def _qc_sample_info(outdata, sampleinfo_raw):
    ...
    _add_usalt_version(outdata, sampleinfo_data)
...
def _config(usalt_config_raw, outdata):
    config_data = _parse_raw_usalt_config_into_dict(usalt_config_raw)
    _add_family_id(config_data, outdata)
    _add_all_samples_from_usalt_config(config_data, outdata)
...
def _add_usalt_version(outdata, sampleinfo_data):
    outdata['usalt_version'] = sampleinfo_data['version']
...
def _parse_raw_usalt_config_into_dict(usalt_config_raw):
    config_data = files.parse_config(usalt_config_raw)
    return config_data
...
def _add_all_samples_from_usalt_config(config_data, outdata):
