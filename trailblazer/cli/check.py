...
@click.pass_context
def mip_check(context: click.Context, family: str):
    ...
    qcmetrics_data = mip_files.parse_qcmetrics(qcmetrics_raw)
    ...
    for sample_data in sampleinfo_data['samples']:
        ...
        with Path(sample_data['chanjo_sexcheck']).open() as chanjo_handle:
            sexcheck_data = mip_files.parse_chanjo_sexcheck(chanjo_handle)
    ...
    if peddy_path.exists():
        with peddy_path.open() as sexcheck_handle:
            peddy_data = mip_files.parse_peddy_sexcheck(sexcheck_handle)
...
@click.argument('family')
@click.pass_context
def balsamic_check(context: click.Context, family: str):
    """Check analysis in database."""
    analysis_obj = context.obj['store'].analyses(family=family).first()
    if analysis_obj is None:
        LOG.error('no analysis found')
    ...
    if not qcmetrics_path.exists():
        ...
        context.abort()
    qcmetrics_raw = ruamel.yaml.safe_load(qcmetrics_path.open())
    qcmetrics_data = balsamic_files.parse_qcmetrics(qcmetrics_raw)
    ...
    for sample_data in sampleinfo_data['samples']:
        ...
        with Path(sample_data['chanjo_sexcheck']).open() as chanjo_handle:
            sexcheck_data = balsamic_files.parse_chanjo_sexcheck(chanjo_handle)
    ...
    if peddy_path.exists():
        with peddy_path.open() as sexcheck_handle:
            peddy_data = balsamic_files.parse_peddy_sexcheck(sexcheck_handle)
...
@click.argument('family')
@click.pass_context
def usalt_check(context: click.Context, family: str):
    """Check analysis in database."""
    analysis_obj = context.obj['store'].analyses(family=family).first()
    if analysis_obj is None:
        LOG.error('no analysis found')
    ...
    if not qcmetrics_path.exists():
        ...
        context.abort()
    qcmetrics_raw = ruamel.yaml.safe_load(qcmetrics_path.open())
    qcmetrics_data = usalt_files.parse_qcmetrics(qcmetrics_raw)
    ...
    for sample_data in sampleinfo_data['samples']:
        ...
        with Path(sample_data['chanjo_sexcheck']).open() as chanjo_handle:
            sexcheck_data = usalt_files.parse_chanjo_sexcheck(chanjo_handle)
    ...
    if peddy_path.exists():
        with peddy_path.open() as sexcheck_handle:
            peddy_data = usalt_files.parse_peddy_sexcheck(sexcheck_handle)
