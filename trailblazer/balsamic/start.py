...
from trailblazer.exc import BalsamicStartError
...
class BalsamicCli(object):

    """Wrapper around Balsamic command line interface."""

    def __init__(self, script):
        """Initialize BALSAMIC command line interface."""
        self.script = script
    ...
    def __call__(self, config, family, **kwargs):
        ...
        if process.returncode != 0:
            raise BalsamicStartError('error starting analysis, check the output')
        return process

    def build_command(self, config, **kwargs):
        """Builds the command to execute Balsamic."""
