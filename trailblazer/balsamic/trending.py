# -*- coding: utf-8 -*-
""" Get BALSAMIC data for trending."""

from trailblazer.balsamic import files


def parse_balsamic_analysis(balsamic_config_raw: dict, qcmetrics_raw: dict, sampleinfo_raw: dict) -> dict:
    """Parse the output analysis files from BALSAMIC for adding info
    to trend database

    Args:
        balsamic_config_raw (dict): raw YAML input from BALSAMIC analysis config file
        qcmetrics_raw (dict): raw YAML input from BALSAMIC analysis qc metric file
        sampleinfo_raw (dict): raw YAML input from BALSAMIC analysis qc sample info file
    Returns:
        dict: parsed data
    """
    outdata = _define_output_dict()

    _config(balsamic_config_raw, outdata)
...
def _qc_sample_info(outdata, sampleinfo_raw):
    ...
    _add_balsamic_version(outdata, sampleinfo_data)
...
def _config(balsamic_config_raw, outdata):
    config_data = _parse_raw_balsamic_config_into_dict(balsamic_config_raw)
    _add_family_id(config_data, outdata)
    _add_all_samples_from_balsamic_config(config_data, outdata)
...
def _add_balsamic_version(outdata, sampleinfo_data):
    outdata['balsamic_version'] = sampleinfo_data['version']
...
def _parse_raw_balsamic_config_into_dict(balsamic_config_raw):
    config_data = files.parse_config(balsamic_config_raw)
    return config_data
...
def _add_all_samples_from_balsamic_config(config_data, outdata):
